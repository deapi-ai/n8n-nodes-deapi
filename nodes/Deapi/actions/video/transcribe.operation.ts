...
const properties: INodeProperties[] = [
  ...
  },
  {
    ...
    description: 'URL of the video to transcribe. YouTube, Twitch VODs, X, and Kick videos are supported.',
    default: '',
    displayOptions: {
      show: {
        source: ['url'],
  ...
  },
  {
    ...
    description: 'The name of the binary field containing the video file. Supported formats: MP4, MPEG, MOV, AVI, WMV, OGG. Max 10 MB.',
    displayOptions: {
      show: {
        source: ['binary'],
      },
  ...
  },
  {
    ...
    options: [
      // {
      //   displayName: 'Return Result in Response',
      //   name: 'returnResultInResponse',
      //   type: 'boolean',
      //   default: false,
      //   description: 'Whether to return the transcription text directly instead of a download URL',
      // },
      {
...
export async function execute(this: IExecuteFunctions, i: number): Promise<INodeExecutionData[]> {
  ...
  // const returnResultInResponse = options.returnResultInResponse as (boolean | undefined) ?? false;
  ...
  if (source === 'url') {
    ...
    const body: VideoToTextRequest = {
      ...
      // return_result_in_response: returnResultInResponse,
    ...
    const { fileContent, contentType, filename } = await getBinaryDataFile(this, i, binaryPropertyName);
    ...
    const request: VideoFileToTextRequest = {
      ...
      include_ts: includeTimestamps,
      model: 'WhisperLargeV3',
      // return_result_in_response: returnResultInResponse || null,
      webhook_url: webhookUrl,
    };
